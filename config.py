...
ENTROPY_COEFF = 0.01
N_EPISODES  = 10000
INNER_STEPS = 36             # 模型内循环固定步数（每个 halting 点内的 MoA+MoE 重复次数）
SAVE_EVERY  = 500
LR          = 1e-3
WEIGHT_DECAY = 0.01
BATCH_SIZE  = 1
...
D_HIDDEN     = 64               # 模型隐藏维度
D_KEY        = 24               # 每 Head 的 Key/Query 维度
D_ATTN_HEAD  = 171               # 每 Attention Head 输出维度
N_ATTN_HEADS = 32              # 注意力 Head 数
N_EXPERTS_PER_HEAD = 32        # 每 Head 统一专家数（含零专家，expert 0 对 Q 为零）

# MoE FFN

D_FFN_HEAD   = 171               # 每 FFN Head 输出维度
N_FFN_HEADS = 32              # MoE FFN 路由头数
N_EXPERTS_PER_FFN_HEAD = 32   # 每头专家数（含零专家）

# Head-wise expert parallelism

N_ATTN_HEADS_PER_CARD = 1     # 每张卡有梯度的 attention head 数（0 = 全部）
N_FFN_HEADS_PER_CARD  = 1     # 每张卡有梯度的 FFN head 数（0 = 全部）
HEAD_ROTATION_INTERVAL = 1  # 每隔 N 步轮换 head 分配（0 = 固定不轮换）
