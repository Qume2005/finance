...
N_SERIES = 2  # 1 train series + 1 test series
...
class MiniKDALayer(nn.Module):
    ...
    def _apply_pope(self, x, positions, is_query):
        ...
        if not is_query:
            ...
            phi = phi + (-2 * np.pi * torch.sigmoid(self.pope_delta_raw))   # (T, dk)
...
print("Model defined: KDAPolicyNetwork")
model = KDAPolicyNetwork().to(DEVICE)
ref_model = KDAPolicyNetwork().to(DEVICE)
...
def grpo_step(policy, ref, opt):
    ...
    with torch.no_grad():
        ...
        dist = Categorical(old_probs)               # (T,)
        actions = dist.sample((G_SAMPLES,))         # (G, T)
    ...
    # KL(π_ref || π_θ)
    with torch.no_grad():
        ref_lp = F.log_softmax(ref(feats), dim=-1)
        ref_p  = ref_lp.exp()
...
for ep in range(1, N_EPISODES + 1):
    loss, rm, rs = grpo_step(model, ref_model, optimizer)
...
for i, sid in enumerate(TEST_IDS):
    r = backtest_series(model, test_feats_by_series[i],
...
Model: Mini-KDA Policy Network (adapted from Kimi Linear)
  ...
  - ~10K parameters total

Training: GRPO (from DeepSeekMath)
  - No critic/value model needed
  - Group-relative advantage estimation (G=16 samples)
